...
// Verificar si el usuario ha iniciado sesión y es de tipo 'Usuario'
if (!isset($_SESSION['usuario']) || $_SESSION['usuario']['rol'] != 'Usuario') {
    // Redirigir al login si no es un usuario
...
$usuario_id = $_SESSION['usuario']['id']; // Asegúrate de que esto existe en tu sesión

// Consulta para obtener los datos del usuario y su póliza
$sql = "
    SELECT 
        u.correo,
        u.rol,
...
if ($result->num_rows > 0) {
    ...
    // Si no encuentra datos, redirigir o mostrar error
...
// Variables para búsqueda (inicializar para evitar errores)
$terminoBusqueda = $_GET['busqueda'] ?? '';
$tipoBusqueda = $_GET['tipo_busqueda'] ?? 'general';
$resultadosBusqueda = []; // Aquí deberías cargar resultados reales de tu BD

$nombre_completo = obtenerNombreCompleto($usuario);
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
        ...
        <div class="container">
            <div class="user-info">
                <div class="user-welcome">
                    Bienvenido, <span><?php echo htmlspecialchars($nombre_completo); ?></span> 
                    (Paciente)
                </div>
        ...
        <div class="content-area">
                ...
                <div class="profile-container">
                    <div class="profile-sidebar">
                        <div class="profile-avatar">
                            <?php 
                                // Mostrar la primera letra del nombre o del correo
                                if (!empty($usuario['nombre'])) {
                                    echo strtoupper(substr($usuario['nombre'], 0, 1));
                                } else {
                                    echo strtoupper(substr($usuario['correo'], 0, 1));
                                }
                            ?>
                        </div>
                    ...
                    <div class="profile-details">
                        <div class="card">
                            ...
                            <div class="info-group">
                                ...
                                <div class="info-value"><?php echo htmlspecialchars($nombre_completo); ?></div>
                            </div>
                            ...
                            <div class="info-group">
                                ...
                                <div class="info-value">
                                    <?php echo !empty($usuario['telefono']) ? htmlspecialchars($usuario['telefono']) : "No disponible"; ?>
                                </div>
                            </div>
                            ...
                            <div class="info-group">
                                ...
                                <div class="info-value">
                                    <?php echo !empty($usuario['fecha_nacimiento']) ? htmlspecialchars($usuario['fecha_nacimiento']) : "No disponible"; ?>
                                </div>
                            </div>
                            ...
                            <div class="info-group">
                                ...
                                <div class="info-value">
                                    <?php echo !empty($usuario['sexo']) ? htmlspecialchars($usuario['sexo']) : "No disponible"; ?>
                                </div>
                            </div>
                            ...
                            <div class="info-group">
                                ...
                                <div class="info-value">
                                    <?php echo !empty($usuario['edad']) ? htmlspecialchars($usuario['edad']) : "No disponible"; ?>
                                </div>
                            </div>
                                ...
                                <div class="info-value">
                                    <?php
                                        ...
                                        if (!empty($usuario['calle'])) {
                                            ...
                                            if (!empty($usuario['num_exterior'])) {
                                                $direccion .= ' #' . $usuario['num_exterior'];
                                            }
                                            if (!empty($usuario['num_interior'])) {
                                                $direccion .= ' Int. ' . $usuario['num_interior'];
                                            }
                                            if (!empty($usuario['colonia'])) {
                                                $direccion .= ', ' . $usuario['colonia'];
                                            }
                                            if (!empty($usuario['codigo_postal'])) {
                                                $direccion .= ', C.P. ' . $usuario['codigo_postal'];
                                            }
                                            if (!empty($usuario['nombre_municipio'])) {
                                                $direccion .= ', ' . $usuario['nombre_municipio'];
                                            }
                                            if (!empty($usuario['nombre_entidad'])) {
                                                $direccion .= ', ' . $usuario['nombre_entidad'];
                                            }
                                        }
                                        echo $direccion ? htmlspecialchars($direccion) : "No disponible";
                                    ?>
                                </div>
                            </div>
                        </div>
            ...
            <section id="poliza" style="display: none;">
                ...
                <div class="policy-card">
                    <div class="policy-badge"><?php echo htmlspecialchars($usuario['estatus'] ?? 'ACTIVA'); ?></div>
                    <h3>Póliza <?php echo htmlspecialchars($usuario['tipo_poliza'] ?? 'Normal'); ?></h3>
                    <p>Número de póliza: <strong><?php echo htmlspecialchars($usuario['poliza_id'] ?? 'No disponible'); ?></strong></p>
                    
                    <div class="policy-features">
                        <h4>Detalles de la póliza:</h4>
                        <ul>
                            <li>Fecha de inicio: <?php echo htmlspecialchars($usuario['fecha_inicio'] ?? 'No disponible'); ?></li>
                            <li>Fecha de fin: <?php echo htmlspecialchars($usuario['fecha_fin'] ?? 'No disponible'); ?></li>
                            <li>Fecha efectiva: <?php echo htmlspecialchars($usuario['fecha_efectiva'] ?? 'No disponible'); ?></li>
                        </ul>
                    </div>
    ...
    <script>
        ...
        function autoCompletarDatos() {
            document.getElementById('nombre').value = '<?php echo htmlspecialchars($nombre_completo); ?>';
            document.getElementById('email').value = '<?php echo htmlspecialchars($usuario['correo']); ?>';
            document.getElementById('telefono').value = '<?php echo htmlspecialchars($usuario['telefono'] ?? ''); ?>';
